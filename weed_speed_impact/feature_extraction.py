...
class ResNet(nn.Module):
    ...
    # 提取融合特征
    def extract(self, data_value):
        # 提取风速特征
        feature_wind_data = data_value[:, 0:2]
        # 提取气体浓度特征
        feature_concentrate_data = data_value[:, 2:]
        # 将气体浓度特征处理为类似图片数据
        feature_concentrate_data = feature_concentrate_data.view(-1, 3, 128, 64)

        # 将数据放到gpu中
        if torch.cuda.is_available():
            feature_wind_data = feature_wind_data.cuda()
            feature_concentrate_data = feature_concentrate_data.cuda()
            self = self.cuda()

        # 数据分割
        gas_diffusion_dataset = GasDiffusionDataset(feature_wind_data, feature_concentrate_data)
        ...
        # 提取特征信息
        start = time()
        value_tensor = torch.zeros(1, 1024)
        for step, (b_w, b_c) in enumerate(train_loader):
            # 提取融合数据特征
            feature_extract_data = self.forward(b_c)
            # 特征融合后的数据特征
            b_x = torch.cat([b_w, feature_extract_data], dim=1)
            value_tensor = torch.cat([value_tensor, b_x.cpu().detach().clone()], dim=0)

        end = time()
        print("特征融合时间为：" + str(end - start) + "秒")
        return value_tensor[1:, :]
