...
class CNNLSTM(nn.Module):
    def __init__(self, input_size):
        ...
        self.conv2 = nn.Sequential(
            nn.Conv2d(100, 50, kernel_size=(18, 8), stride=1),
            nn.BatchNorm2d(50),
        )
    ...
    def train_model(self, train_x, train_y, loss_rate=0.2, verify_rate=0.1):
        # 获取训练集 验证集比例
        ...
        for epoch in range(num_epochs):
            ...
            if epoch % 10 == 0:
                ...
                for step, (v_x, v_y) in enumerate(verify_loader):
                    ...
                    # 获取验证集损失
                    verify_loss_bce = criterion_bce(predict, b_y)
                    verify_loss_kl = criterion_kl(predict, b_y)
                ...
                if verify_loss_sum < best_res:
                    ...
                    torch.save(self.state_dict(), './model/cnn_lstm_se_best.pt')
            scheduler.step()
        end = time()
        print("训练时间为：" + str(end - start) + "秒")

        # 保存模型
        torch.save(self.state_dict(), './model/cnn_lstm_se.pt')
